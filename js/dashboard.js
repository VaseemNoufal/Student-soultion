document.addEventListener("DOMContentLoaded", () => {
    ...
    function resetTimer() {
        ...
        timeInSeconds = parseInt(
            document.querySelector(".time-btn.active")?.dataset.time || "1500"
        );
        updateTimerDisplay();
        startPauseBtn.innerHTML = "▶";
    }
    ...
    document
        .querySelector('.time-btn[data-time="1500"]')
    ...
    async function fetchTasks() {
        try {
            const response = await fetch(
                "http://85.215.229.78:8447/api/v1/homeworks/manage/",
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                    },
            ...
            console.log(tasks);
            renderTask();
            console.log("7. renderTasks() call completed");
        } catch (err) {
    ...
    function renderTask() {
        console.log("Inisde Render", tasks);
        taskList.innerHTML = "";
        ...
        if (filteredTasks.length === 0) {
            taskList.innerHTML =
                '<p class="no-tasks-msg">No tasks here. Add one above!</p>';
            return;
        }
            ...
            taskCard.innerHTML = `
                <div class="task-info">
                    ...
                    <div class="task-meta">
                        ...
                        ${task.subject_name && task.due_date ? " &bull; " : ""}
                        ${
                            task.due_date
                                ? `<span>${new Date(
                                      task.due_date
    ...
    async function addTask() {
        ...
        try {
            await fetch("http://85.215.229.78:8447/api/v1/homeworks/create/", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${accessToken}`,
    ...
    const calendarModalEventList = document.getElementById(
    ...
    function renderCalendarGrid() {
        ...
        monthLabel.textContent = now.toLocaleString("default", {
            month: "long",
            year: "numeric",
        });
    ...
    function renderCalendarModalEvents() {
        ...
        events.forEach((ev, idx) => {
            ...
            delBtn.textContent = "❌";
            delBtn.onclick = () => {
                const i = calendarEvents.findIndex(
                    (e) =>
                        e.date === selectedCalendarDate && e.title === ev.title
                );
                if (i > -1) {
    ...
    const simpleTimerMinutesInput = document.getElementById(
        "simple-timer-minutes"
    );
    ...
    function updateSimpleTimerDisplay() {
        ...
        simpleTimerDisplay.textContent = `${min < 10 ? "0" : ""}${min}:${
            sec < 10 ? "0" : ""
        }${sec}`;
    }
    ...
    function updateExamStats() {
        ...
        userSubjects.forEach((subject) => {
            if (examData[subject] && examData[subject].tasks) {
                ...
                completedChapters += examData[subject].tasks.filter(
                    (task) => task.completed
                ).length;
            }
        });
            ...
            card.innerHTML = `
                    ...
                    <div class="subject-card-title">
                        ...
                        <div class="subject-card-info">
                            ...
                            <div class="subject-card-meta">
                                <span class="subject-meta-tag">${
                                    subjectData.className || "Class 10"
                                }</span>
                                ${
                                    subjectData.examDate
                                        ? `<span class="subject-meta-tag exam-date">📅 ${subjectData.examDate}</span>`
                                        : ""
                                }
                            </div>
                        </div>
                ...
                <div class="subject-progress-section">
                    <div class="progress-header">
                        <span class="progress-text">${completedTasks} of ${
                tasks.length
            } chapters completed</span>
                ...
                ${
                    noDate
                        ? `<div style='color:#ef4444;font-size:0.95rem;margin-bottom:1rem;padding:0.8rem;background:#fef2f2;border-radius:0.8rem;border:1px solid #fecaca;'>⚠️ Please set the exam date before adding chapters.</div>`
                        : ""
                }
                <form class="add-chapter-form" onsubmit="addChapterTask(event, '${subjectName}')">
                    <input type="text" class="modern-chapter-input" placeholder="Quick add chapter..." required ${
                        noDate ? "disabled" : ""
                    }>
    ...
    function renderTasks(subject) {
        ...
        if (tasks.length === 0) {
            checklist.innerHTML =
                '<li style="color: #64748b; font-style: italic; text-align: center; padding: 2rem;">No chapters added yet. Add your first chapter above!</li>';
            return;
        }

        checklist.innerHTML = tasks
            .map(
                (task, index) => `
            <li class="modern-chapter-item ${
    ...
    function toggleTask(subject, index) {
        examData[subject].tasks[index].completed =
            !examData[subject].tasks[index].completed;
        saveExamData();
        renderTasks(subject);
        updateExamStats();
    ...
    function editTask(subject, index) {
        const newText = prompt(
            "Edit chapter:",
            examData[subject].tasks[index].text
        );
    ...
    function generateAISuggestions(subject) {
        ...
        if (aiBox.style.display === "none") {
            ...
            const availableChapters =
                syllabusChapters[className]?.[syllabus]?.[subject] || [];
    ...
    function addChapterAsTask(subject, chapter) {
        ...
        const exists = examData[subject].tasks.some(
            (task) => task.text === chapter
        );
        if (exists) {
    ...
    function removeSubject(subjectName) {
        if (
            confirm(
                `Are you sure you want to remove "${subjectName}" and all its chapters?`
            )
        ) {
    ...
    if (subjectForm) {
        subjectForm.addEventListener("submit", (e) => {
            ...
            const subjectName = document
                .getElementById("modalSubjectName")
                .value.trim();
    ...
    if (chapterForm) {
        chapterForm.addEventListener("submit", (e) => {
            ...
            // Check if chapter already exists
            const exists = examData[window.currentSubject].tasks.some(
                (task) => task.text === chapterName
            );
            if (exists) {
                alert("This chapter is already added!");
                return;
    ...
    function renderTimetableTable() {
        ...
        timetableDays.forEach((day, dayIdx) => {
            ...
            for (let p = 1; p <= timetablePeriods; p++) {
                ...
                html += `<td><input type="text" id="${cellId}" class="form-input timetable-input" placeholder="Subject" value="${
                    (timetableData[day.id] && timetableData[day.id][p]) || ""
                }" oninput="updateTimetableCell(${
                    day.id
                },${p},this.value)"></td>`;
            }
            html += "</tr>";
    ...
    window.removePeriod = function (periodNum) {
        if (timetablePeriods > 1) {
            ...
            for (const day of timetableDays) {
                if (timetableData[day.id])
                    delete timetableData[day.id][timetablePeriods + 1];
            }
            renderTimetableTable();
        }
    };
    ...
    function addDay() {
        const nextId =
            timetableDays.length > 0
                ? Math.max(...timetableDays.map((d) => d.id)) + 1
                : 1;
